    ...
    try {
      ...
      if (observersStr) {
        try {
          ...
          const { data: dbObs } = await supabase.from('observers').select('id, nombre');
          const existingNames = new Set(
            (dbObs || []).map(o => o.nombre.trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''))
          );

          for (const o of obsList) {
            const norm = o.nombre.trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
