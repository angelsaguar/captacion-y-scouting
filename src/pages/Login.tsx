...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!isAdmin && !isSanti && !isAlejandro && !isJavier) {
      toast.error('Acceso restringido: Solo el administrador Ángel Saguar y los observadores autorizados (SANTI, ALEJANDRO SAGUAR, JAVIER ASENSIO) tienen permitido el acceso.');
      return;
    }
    ...
    try {
      ...
      if (data?.user) {
        ...
        const loggedIsJavier = loggedEmail.includes('javier') || loggedEmail.includes('asensio') || loggedNombre.includes('javier') || loggedNombre.includes('asensio');
        const loggedIsAdmin = loggedEmail === 'angel.saguar@telefonica.net';

        if (!loggedIsAdmin && !loggedIsSanti && !loggedIsAlejandro && !loggedIsJavier) {
