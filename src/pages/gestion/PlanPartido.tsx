...
export default function PlanPartido() {
  ...
  const handleGenerateAiCounterStrategy = async () => {
    if (!formData.sistemaRival) {
      toast.error('Selecciona el sistema de juego del equipo rival para analizar la contra-estrategia.');
      return;
    }
    ...
    try {
      ...
      if (!alineacion && roster.length > 0) {
        ...
        alineacion = `Portería: ${gks[0]?.nombre || 'Titular'}\nDefensa: ${dfs.map(d => d.nombre).slice(0, 4).join(', ') || 'Titulares'}\nMediocampo: ${mfs.map(m => m.nombre).slice(0, 3).join(', ') || 'Titulares'}\nDelantera: ${fws.map(f => f.nombre).slice(0, 3).join(', ') || 'Titulares'}`;
      }

      setFormData(prev => ({
        ...prev,
        sistema: data.sistemaRecomendado || prev.sistema,
        objetivos_tacticos: Array.isArray(data.objetivosTacticos) 
          ? data.objetivosTacticos.join('\n') 
          : (data.objetivosTacticos || data.razonamientoSistema),
        puntos_fuertes_rival: (data.razonamientoSistema ? `Análisis táctico: ${data.razonamientoSistema}\n\n` : '') + 
          (Array.isArray(data.puntosFuertesRival) ? data.puntosFuertesRival.join('\n') : (data.puntosFuertesRival || '')),
        alineacion_propuesta: alineacion,
        balon_parado: data.estrategiaBalonParado || 'Córners ofensivos y faltas con centros tensos.'
      }));
