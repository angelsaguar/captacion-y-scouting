...
export default function Partidos() {
  ...
  // Plan de Partido & IA Táctica Modal state
  const [showPlanPartidoModal, setShowPlanPartidoModal] = useState<Match | null>(null);
  const [planRivalName, setPlanRivalName] = useState<string>('');
  const [planRivalSystem, setPlanRivalSystem] = useState<string>('1-4-4-2');
  const [planRivalNotes, setPlanRivalNotes] = useState<string>('');
  const [planMySystem, setPlanMySystem] = useState<string>('1-4-3-3');
  const [planTacticalAdvice, setPlanTacticalAdvice] = useState<{
  ...
  const [isAnalyzingTactics, setIsAnalyzingTactics] = useState<boolean>(false);
  const [savedMatchPlanId, setSavedMatchPlanId] = useState<string | null>(null);

  const handleOpenPlanPartidoModal = async (match: Match | null) => {
    ...
    setShowPlanPartidoModal(targetMatch);
    setPlanRivalName(targetMatch.rival || '');
    setPlanRivalSystem('1-4-4-2');
    setPlanRivalNotes('');
    setPlanMySystem('1-4-3-3');
    setPlanTacticalAdvice(null);
    setSavedMatchPlanId(null);

    if (targetMatch.rival) {
      try {
        const { data } = await supabase
          .from('match_plans')
          .select('*')
          .eq('team', selectedTeam)
          .eq('rival_name', targetMatch.rival)
          .limit(1);

        if (data && data.length > 0) {
          const plan = data[0];
          setSavedMatchPlanId(plan.id);
          setPlanRivalSystem(plan.sistema_rival || '1-4-4-2');
          setPlanMySystem(plan.sistema || '1-4-3-3');
          setPlanTacticalAdvice({
            sistemaRecomendado: plan.sistema || '1-4-3-3',
            razonamientoSistema: plan.puntos_fuertes_rival || '',
            objetivosTacticos: plan.objetivos_tacticos ? plan.objetivos_tacticos.split('\n') : [],
            puntosFuertesRival: [],
            instruccionesPorPuesto: plan.alineacion_propuesta || '',
            estrategiaBalonParado: plan.balon_parado || ''
          });
        }
      } catch (e) {
        console.warn('Could not fetch existing match plan:', e);
      }
    }
  };

  const handleGenerateAiTacticsInPartidos = async () => {
    if (!planRivalSystem) {
      toast.error('Selecciona el sistema con el que juega el equipo rival.');
      return;
    }
  ...
  const handleSavePlanPartidoFromModal = async () => {
    if (!showPlanPartidoModal) return;
    const rival = planRivalName || showPlanPartidoModal.rival || 'Rival Genérico';
    const matchDate = showPlanPartidoModal.fecha || new Date().toISOString().split('T')[0];

    const payload = {
      team: selectedTeam,
      rival_name: rival,
      fecha_partido: matchDate,
      sistema: planMySystem,
      sistema_rival: planRivalSystem,
      objetivos_tacticos: planTacticalAdvice ? (Array.isArray(planTacticalAdvice.objetivosTacticos) ? planTacticalAdvice.objetivosTacticos.join('\n') : planTacticalAdvice.objetivosTacticos) : '',
      alineacion_propuesta: planTacticalAdvice ? planTacticalAdvice.instruccionesPorPuesto : '',
      puntos_fuertes_rival: planTacticalAdvice ? planTacticalAdvice.razonamientoSistema : '',
      balon_parado: planTacticalAdvice ? planTacticalAdvice.estrategiaBalonParado : '',
    };

    const toastId = toast.loading('Guardando plan de partido...');

    try {
      if (savedMatchPlanId) {
        await supabase.from('match_plans').update(payload).eq('id', savedMatchPlanId);
      } else {
        const { data, error } = await supabase.from('match_plans').insert([payload]).select();
        if (!error && data && data[0]) {
          setSavedMatchPlanId(data[0].id);
        }
      }

      // Sync local storage
      const localKey = `team_gameplans_${selectedTeam}`;
      const existingStr = localStorage.getItem(localKey);
      const existing = existingStr ? JSON.parse(existingStr) : [];
      const updated = [
        {
          id: savedMatchPlanId || crypto.randomUUID(),
          rivalName: rival,
          fechaPartido: matchDate,
          sistema: planMySystem,
          sistemaRival: planRivalSystem,
          objetivos_tacticos: payload.objetivos_tacticos,
          alineacion_propuesta: payload.alineacion_propuesta,
          puntos_fuertes_rival: payload.puntos_fuertes_rival,
          balon_parado: payload.balon_parado,
          fecha_creacion: new Date().toISOString().split('T')[0]
        },
        ...existing.filter((p: any) => p.rivalName !== rival)
      ];
      localStorage.setItem(localKey, JSON.stringify(updated));

      toast.success('¡Plan de partido guardado correctamente en la base de datos!', { id: toastId });
    } catch (err) {
      console.warn('Error saving match plan:', err);
      toast.success('Plan de partido guardado localmente.', { id: toastId });
    }
  };

  const handleSharePlanWhatsapp = () => {
    if (!showPlanPartidoModal) return;
    const rival = planRivalName || showPlanPartidoModal.rival || 'Rival';
    const sysRival = planRivalSystem;
    const sysMine = planMySystem;
    
    let msg = `📋 *PLAN DE PARTIDO & ESTRATEGIA TÁCTICA IA* 📋\n\n`;
    msg += `⚽ *${selectedTeam} vs ${rival}*\n`;
    msg += `🛡️ *Sistema del Rival:* ${sysRival}\n`;
    msg += `⚡ *Nuestro Sistema Recomendado (IA):* ${sysMine}\n\n`;

    if (planTacticalAdvice) {
      msg += `💡 *Análisis Táctico de Neutralización:*\n${planTacticalAdvice.razonamientoSistema}\n\n`;
      if (planTacticalAdvice.objetivosTacticos?.length) {
        msg += `🎯 *Objetivos Clave:*\n${planTacticalAdvice.objetivosTacticos.map(o => `• ${o}`).join('\n')}\n\n`;
      }
      if (planTacticalAdvice.estrategiaBalonParado) {
        msg += `📐 *Estrategia ABP:*\n${planTacticalAdvice.estrategiaBalonParado}\n\n`;
      }
    }
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-slate-900/60 border border-slate-900 p-4 rounded-2xl">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          ...
          <Button 
            ...
            className="text-xs font-bold uppercase tracking-wider bg-blue-600 hover:bg-blue-500 flex items-center justify-center gap-2 h-10 px-4 rounded-xl cursor-pointer"
          >
              ...
              >
                ...
                <div className="border-t border-slate-900/60 pt-3 flex flex-wrap items-center justify-between gap-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Button
                      onClick={() => handleOpenPlanPartidoModal(match)}
                      className="text-[10px] font-black text-amber-300 bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/30 flex items-center gap-1.5 px-3 h-8 rounded-xl uppercase transition-all cursor-pointer shadow-sm"
                    >
      ...
      {/* Plan de Partido & IA Táctica Modal */}
      {showPlanPartidoModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md overflow-y-auto">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-4xl my-8 overflow-hidden shadow-2xl animate-in fade-in duration-200 text-white">
            ...
            <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-950/80">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-extrabold text-white text-base uppercase tracking-tight flex items-center gap-2">
                    <span>Plan de Partido & Contra-Estrategia IA</span>
                    <span className="bg-amber-500/20 text-amber-300 text-[10px] font-black px-2 py-0.5 rounded-full border border-amber-500/30">
                      GEMINI AI
                    </span>
                  </h3>
                  <p className="text-xs text-slate-400 font-medium">
                    {selectedTeam} {planRivalName ? `vs ${planRivalName}` : '• Selección de Sistema Rival & Contra-Medidas'}
                  </p>
                </div>
              </div>
              ...
              <button 
                type="button"
                onClick={() => setShowPlanPartidoModal(null)}
                className="text-slate-400 hover:text-white text-lg font-bold w-8 h-8 rounded-xl bg-slate-800 flex items-center justify-center cursor-pointer"
              >
                ✕
              </button>
            </div>
            ...
            <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
              ...
              <div className="bg-slate-950/60 p-5 rounded-2xl border border-slate-800 space-y-4">
                <h4 className="text-xs font-black text-amber-400 uppercase tracking-wider flex items-center gap-2 border-b border-slate-800 pb-2">
                  ...
                  <span>1. Configuración del Encuentro y Sistema Rival</span>
                </h4>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="text-[11px] font-bold text-slate-400 uppercase">Equipo Rival</label>
                    <input 
                      type="text"
                      value={planRivalName}
                      onChange={(e) => setPlanRivalName(e.target.value)}
                      placeholder="Ej. A.D. Arganda"
                      className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-xs text-white focus:outline-none focus:border-amber-500 mt-1 font-bold"
                    />
                  </div>

                  <div>
                    <label className="text-[11px] font-bold text-slate-400 uppercase">Sistema / Formación del Rival *</label>
                    <select
                      value={planRivalSystem}
                      onChange={(e) => setPlanRivalSystem(e.target.value)}
                      className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-xs text-white focus:outline-none focus:border-amber-500 mt-1 font-bold cursor-pointer"
                    >
                      <option value="1-4-4-2">1-4-4-2 (Doble pivote plano)</option>
                      <option value="1-4-3-3">1-4-3-3 (Pivote y 2 interiores)</option>
                      <option value="1-4-2-3-1">1-4-2-3-1 (Doble pivote y enganche)</option>
                      <option value="1-3-5-2">1-3-5-2 / 1-5-3-2 (3 Centrales y carrileros)</option>
                      <option value="1-3-4-3">1-3-4-3 (Rombo / Tridente ofensivo)</option>
                      <option value="1-4-1-4-1">1-4-1-4-1 (Bloque denso)</option>
                      <option value="1-4-5-1">1-4-5-1 (Pivote defensivo)</option>
                    </select>
                  </div>

                  <div>
                    <label className="text-[11px] font-bold text-slate-400 uppercase">Nuestro Sistema Inicial</label>
                    <select
                      value={planMySystem}
                      onChange={(e) => setPlanMySystem(e.target.value)}
                      className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-xs text-white focus:outline-none focus:border-amber-500 mt-1 font-bold cursor-pointer"
                    >
                      <option value="1-4-3-3">1-4-3-3 (Recomendado habitual)</option>
                      <option value="1-4-2-3-1">1-4-2-3-1 (Equilibrio con mediapunta)</option>
                      <option value="1-4-4-2">1-4-4-2 (Doble punta)</option>
                      <option value="1-3-5-2">1-3-5-2 (Carrileros largos)</option>
                      <option value="1-5-3-2">1-5-3-2 (Bloque sólido)</option>
                    </select>
                  </div>
                </div>

                <div>
                  <label className="text-[11px] font-bold text-slate-400 uppercase">Estilo / Fortalezas del Rival (Opcional)</label>
                  <textarea
                    value={planRivalNotes}
                    onChange={(e) => setPlanRivalNotes(e.target.value)}
                    placeholder="Ej. Salen jugando desde atrás por bajo, sus dos bandas son muy veloces en contraataque, sufren en balones colgados al área..."
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-xs text-white focus:outline-none focus:border-amber-500 mt-1 h-14"
                  />
                </div>

                {/* Gemini AI Trigger Button */}
                <Button
                  onClick={handleGenerateAiTacticsInPartidos}
                  disabled={isAnalyzingTactics}
                  className="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-slate-950 font-black uppercase text-xs tracking-wider py-3 h-11 rounded-xl shadow-lg flex items-center justify-center gap-2 cursor-pointer transition-all"
                >
                  <Sparkles className="w-4 h-4 text-slate-950 fill-current animate-bounce" />
                  <span>
                    {isAnalyzingTactics 
                      ? 'Analizando con IA Gemini y calculando contra-sistema...' 
                      : `✨ Asesor Táctico IA (Gemini): Calcular cómo contra-restar el ${planRivalSystem}`
                    }
                  </span>
                </Button>
              </div>
              ...
              {planTacticalAdvice && (
                <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  ...
                  <div className="bg-gradient-to-r from-amber-950/40 via-slate-900 to-slate-950 border border-amber-500/40 p-5 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xl">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <ShieldCheck className="w-5 h-5 text-amber-400" />
                        <span className="text-xs font-black uppercase text-amber-400 tracking-wider">Sistema Contra-Estrategia Recomendado por la IA</span>
                      </div>
                      <div className="text-2xl font-black text-white flex items-center gap-3">
                        <span>{planTacticalAdvice.sistemaRecomendado}</span>
                        <span className="text-xs font-bold text-slate-400 bg-slate-800 px-3 py-1 rounded-full uppercase border border-slate-700">
                          Contra el {planRivalSystem} rival
                        </span>
                      </div>
                    </div>
                    ...
                    <Button
                      type="button"
                      onClick={() => {
                        setPlanMySystem(planTacticalAdvice.sistemaRecomendado);
                        toast.success(`Sistema adaptado a ${planTacticalAdvice.sistemaRecomendado}`);
                      }}
                      className="bg-amber-500 text-slate-950 font-extrabold text-xs uppercase px-4 h-9 rounded-xl hover:bg-amber-400 cursor-pointer"
                    >
                      Aplicar {planTacticalAdvice.sistemaRecomendado}
                    </Button>
                  </div>

                  {/* Neutralization Rationale */}
                  <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800 space-y-2">
                    <h5 className="text-xs font-black uppercase text-amber-400 flex items-center gap-1.5">
                      <Zap className="w-4 h-4 text-amber-400" />
                      <span>Análisis Táctico de Neutralización (¿Por qué este sistema?)</span>
                    </h5>
                  ...
                  {/* Objectives & Rival Strengths Grid */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800 space-y-2">
                      <h5 className="text-xs font-black uppercase text-emerald-400 flex items-center gap-1.5">
                        <Target className="w-4 h-4 text-emerald-400" />
                        <span>Objetivos Tácticos Clave</span>
                      </h5>
                      <ul className="space-y-1.5 text-xs text-slate-300">
                        {Array.isArray(planTacticalAdvice.objetivosTacticos) ? (
                          planTacticalAdvice.objetivosTacticos.map((obj, i) => (
                          ...
                          <p className="text-xs text-slate-300 whitespace-pre-wrap">{planTacticalAdvice.objetivosTacticos}</p>
                        )}
                      </ul>
                    </div>

                    <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800 space-y-2">
                      <h5 className="text-xs font-black uppercase text-rose-400 flex items-center gap-1.5">
                        <AlertTriangle className="w-4 h-4 text-rose-400" />
                        <span>Vigilancias & Peligros del Rival</span>
                      </h5>
                      <ul className="space-y-1.5 text-xs text-slate-300">
                        {Array.isArray(planTacticalAdvice.puntosFuertesRival) ? (
                          planTacticalAdvice.puntosFuertesRival.map((pt, i) => (
                            <li key={i} className="flex items-start gap-2 bg-slate-900/80 p-2 rounded-lg border border-slate-800/80">
                          ...
                          <p className="text-xs text-slate-300 whitespace-pre-wrap">{planTacticalAdvice.puntosFuertesRival}</p>
                        )}
                      </ul>
                    </div>
                  </div>

                  {/* Lineup & Role Instructions */}
                  <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800 space-y-2">
                    <h5 className="text-xs font-black uppercase text-blue-400 flex items-center gap-1.5">
                      <Users className="w-4 h-4 text-blue-400" />
                      <span>Instrucciones Específicas por Puesto</span>
                    </h5>
                    <textarea
                      value={planTacticalAdvice.instruccionesPorPuesto}
                      onChange={(e) => setPlanTacticalAdvice({...planTacticalAdvice, instruccionesPorPuesto: e.target.value})}
                      className="w-full bg-slate-900 border border-slate-800 rounded-xl p-3 text-xs text-slate-200 focus:outline-none focus:border-amber-500 h-24"
                    />
                  </div>

                  {/* ABP / Set Pieces */}
                  <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800 space-y-2">
                    <h5 className="text-xs font-black uppercase text-orange-400 flex items-center gap-1.5">
                      <Award className="w-4 h-4 text-orange-400" />
                      <span>Estrategia a Balón Parado (ABP)</span>
                    </h5>
                    <textarea
                      value={planTacticalAdvice.estrategiaBalonParado}
                      onChange={(e) => setPlanTacticalAdvice({...planTacticalAdvice, estrategiaBalonParado: e.target.value})}
                      className="w-full bg-slate-900 border border-slate-800 rounded-xl p-3 text-xs text-slate-200 focus:outline-none focus:border-amber-500 h-20"
                    />
                  </div>
                </div>
              )}
            ...
            <div className="p-6 border-t border-slate-800 bg-slate-950/80 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <Button
                  type="button"
                  variant="outline"
                  onClick={handleSharePlanWhatsapp}
                  className="text-xs font-bold border-green-600/40 text-green-400 hover:bg-green-600/20 rounded-xl gap-2 h-10 cursor-pointer"
                >
                  <MessageSquare className="w-4 h-4 text-green-400 fill-current" />
              ...
              <div className="flex items-center gap-3">
                <Button
                  ...
                  variant="outline"
                  onClick={() => setShowPlanPartidoModal(null)}
                  className="text-xs font-bold border-slate-800 text-slate-400 hover:bg-slate-800 rounded-xl h-10 cursor-pointer"
                >
                  Cerrar
                </Button>
                ...
                <Button
                  type="button"
                  onClick={handleSavePlanPartidoFromModal}
                  className="text-xs font-black uppercase bg-amber-500 hover:bg-amber-400 text-slate-950 rounded-xl gap-2 cursor-pointer shadow-lg px-5 h-10"
                >
                  <Save className="w-4 h-4" />
                  <span>Guardar Plan de Partido</span>
                </Button>
              </div>
            </div>
