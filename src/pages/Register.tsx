...
import { Trophy, Mail, Lock, Loader2, UserPlus, FileText, Database, ShieldCheck, KeySquare } from 'lucide-react';
...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isJavier = cleanEmail.includes('javier') || cleanNombre.includes('javier') || cleanEmail.includes('asensio') || cleanNombre.includes('asensio');
      const isAdmin = cleanEmail === 'angel.saguar@telefonica.net';

      if (!isAdmin && !isSanti && !isAlejandro && !isJavier) {
        throw new Error('Acceso restringido: Solo se permite el registro al administrador Ángel Saguar y a los observadores autorizados (SANTI, ALEJANDRO SAGUAR, JAVIER ASENSIO).');
      }
